...
def cat(x1, y1, x2, y2, colorc, colory, angle):
    '''
    ...
    colorc - цвет кота
    colory - цвет глаз
    angle - угол поворота кота относительно верхней левой точки экрана

    ВАЖНО!!!
    координата y2 обязательно должна быть больше y1
    '''
    # создание поверхности котика
    cat_surface = pygame.Surface((700, 700), pygame.SRCALPHA, 32)
  
    a = y2 - y1
    b = x2 - x1
    if b >= 0:
        # нога за головой
        ellipse(cat_surface, colorc,
            (x1 + b / 20, y1 + a / 2, b / 8, a / 2))  
        ellipse(cat_surface, WHITE,
        ...
        # голова 
        circle(cat_surface, colorc,
           (x1 + b / 6, y1 + a / 2), a / 3) 
        circle(cat_surface, WHITE,
           (x1 + b / 6, y1 + a / 2), a / 3, 2)
        ...
        # глаза
        for i in (-1, 1):
            circle(cat_surface, colory,
               (x1 + b / 6 + i * b / 15, y1 + a / 2 - a / 10), a / 14)  
            circle(cat_surface, BLACK,
               (x1 + b / 6 + i * b / 15, y1 + a / 2 - a / 10), a / 14, 1)
            ellipse(cat_surface, BLACK,
    ...
    # рисуем усы
    for i in range(5):
        line(cat_surface, WHITE, [x1 + b / 6 + b / 30, y1 + a / 2 + a / 20 + a / 50 * i],
    ...
    # поворот кота
    catsur=pygame.transform.rotate(cat_surface, angle)
    sc.blit(catsur, (0, 0))
...
# задаем положение, размер, цвет и угол поворота котов
cat(300, 300, 500, 400, GRAY, BLUE, 0)
cat(500, 300, 600, 350, ORANGE, GREEN, 0)
cat(100, 500, 300, 600, GRAY, BLUE, 0)
cat(250, 250, 50, 350, ORANGE, GREEN, 0)
cat(500, 500, 400, 550, ORANGE, GREEN, 0)
